...
@interface LLCalendarView() {
    ...
    UIButton *previousWeekButton;//上次选中的周按钮
    UIButton *currentWeekButton;//当期那选中的周按钮
}
...
@property (nonatomic, strong)NSDate *currentWeekStartDate;
@property (nonatomic, strong)NSDate *nextWeekStartDate;
@property (nonatomic, strong)NSDate *nextTwoWeekStartDate;
...
//某年某月某日的0点日期
- (NSDate *)dateWithYear:(int32_t)year month:(int32_t)month day:(int32_t)day
{
    //字符串转换为日期
    //实例化一个NSDateFormatter对象
    NSDateFormatter* dateFormat = [[NSDateFormatter alloc] init];
    //设定时间格式,这里可以设置成自己需要的格式
    [dateFormat setDateFormat:@"yyyy-MM-dd"];
    NSDate *date =[dateFormat dateFromString:[NSString stringWithFormat:@"%@-%@-%@",@(year),@(month),@(day)]];
    
    return date;
}

- (void)setYearMonthDayWithDate:(NSDate *)date
{
    self.year = (int32_t)date.year;
    self.month = (int32_t)date.month;
    self.day = (int32_t)date.day;
}
...
- (void)_initTitleButtons
{
    NSArray *buttonTitles = @[@"本周", @"下周", @"下下周"];
    CGFloat buttonWidth = CGRectGetWidth(_contentBgView.frame)/3;
    for (int i = 0; i < 3; i++) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(buttonWidth*i, 0, buttonWidth, 42);
        [button setTitle:buttonTitles[i] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        button.tag = i + 100;
        [button setTitleColor:[UIColor colorWithRed:0.365 green:0.698 blue:0.765 alpha:1.000] forState:UIControlStateSelected];
        [button addTarget:self action:@selector(titleButtonAction:) forControlEvents:UIControlEventTouchUpInside];
        [_contentBgView addSubview:button];
...
{
    if (button == previousWeekButton) {
        return;
    }
    button.selected = !button.selected;
    previousWeekButton.selected = NO;
    
    switch (button.tag - 100) {
        case 0:
            [self currentWeekSwitch];
            break;
        case 1:
            [self nextWeekSwitch];
            break;
        case 2:
            [self nextTwoWeetSwitch];
            break;
...
- (void)_initDate
{
    //获取起始日期
    NSDate *currentDate = [NSDate date];
    NSDate *startDate = [currentDate firstDateOfWeek];
    
    [self setYearMonthDayWithDate:startDate];
    
    self.currentWeekStartDate = startDate;
}

- (void)currentWeekSwitch
{
    if (self.currentWeekStartDate) {
        [self setYearMonthDayWithDate:self.currentWeekStartDate];
    }
...
- (void)nextWeekSwitch
{
    if (self.nextWeekStartDate) {
        [self setYearMonthDayWithDate:self.nextWeekStartDate];
    }
    else
    {
        if (self.month < 12) {
            if (previousWeekButton.tag == 100) {
                self.day += 7;
            }
            else
            {
        ...
        self.nextWeekStartDate = [self dateWithYear:self.year month:self.month day:self.day];
    }
    
    [self refreshDateTitle];
}

- (void)nextTwoWeetSwitch
{
    if (self.nextTwoWeekStartDate) {
        [self setYearMonthDayWithDate:self.nextTwoWeekStartDate];
    }
    else
    {
        if (self.month < 12) {
        ...
        self.nextTwoWeekStartDate = [self dateWithYear:self.year month:self.month day:self.day];
    }
...
- (void)showDateView {
    ...
    NSArray *tmparr = @[@"周一",@"周二",@"周三",@"周四",@"周五",@"周六",@"周日"];
    for(int i = 0 ;i < 7; i++)
    {
    ...
    //设定时间格式,这里可以设置成自己需要的格式
    [dateFormat setDateFormat:@"yyyy-MM-dd"];
    NSDate *firstDay =[dateFormat dateFromString:[NSString stringWithFormat:@"%@-%@-%@",@(self.year),@(self.month),@(self.day)]];
    ...
    for(int i = startDayIndex; i < kTotalNum;i++)
    {
        ...
        NSString *title = [NSString stringWithFormat:@"%@-%@", INTTOSTR(date.month), INTTOSTR(date.day)];
        if ([date isToday])
        {//今天
//            title = @"今天";
